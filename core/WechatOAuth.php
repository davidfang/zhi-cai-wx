...
class WechatOAuth{
    ...
    public static function getCode($redirect_uri, $state=1, $scope='snsapi_base'){
        ...
        //Msg::saveWeixinErrMsg(555,'网页授权redirect_uri错误'.$redirect_uri,__FILE__,__LINE__,'');
        //返回类型，请填写code
        $response_type = 'code';
        //构造请求微信接口的URL
    ...
    public static function getUserInfo($accessToken, $openId, $lang='zh_CN'){
        ...
        if(empty($user_info)){//用户不存在的情况下，直接去腾讯接口请求
            ...
            $queryAction = 'GET';
            $user_info = Curl::callWebServer($queryUrl, '', $queryAction);
            if(isset($user_info['errcode'])){
                //throw  '获取用户信息错误';
                $line_code = '$user_info = Curl::callWebServer('.$queryUrl.", '',". $queryAction.');';
                Msg::saveWeixinErrMsg($user_info['errcode'],$user_info['errmsg'],__FILE__,__LINE__-4,$line_code);
                //return false;
